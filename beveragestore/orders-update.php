...
if(isset($_SESSION['cart'])) {
  ...
  foreach($_SESSION['cart'] as $product_id => $quantity) {
    ...
    $result = $connect->query("SELECT * FROM products WHERE id = ".$product_id);
    ...
    if($result){
      ...
      if($obj = $result->fetch_object()) {
        ...
        $query = $connect->query("INSERT INTO orders (product_code, product_name, product_desc, price, units, total, email) VALUES('$obj->product_code', '$obj->product_name', '$obj->product_desc', $obj->price, $quantity, $cost, '$user')");

        if($query){
          $newqty = $obj->qty - $quantity;
          if($connect->query("UPDATE products SET qty = ".$newqty." WHERE id = ".$product_id)){
